...
\begin{titlepage}
  ...
  \vfill
  {\large Version LaTeX académique -- \today}
\end{titlepage}
...
Je remercie également toutes les personnes qui ont contribué aux tests, aux remarques et à l'amélioration progressive de l'application \textbf{Mon PF App}. Ce projet m'a permis de rapprocher les acquis de programmation, base de données, analyse UML, sécurité, tests et documentation d'un besoin applicatif concret : la gestion des commandes d'un restaurant.

\chapter*{Résumé}
\addcontentsline{toc}{chapter}{Résumé}
Ce rapport présente \textbf{Mon PF App}, une application mobile et multi-plateforme de gestion des commandes d'un restaurant. Le système permet à un client de consulter un menu, gérer un panier, passer une commande et suivre son état. Il propose aussi des espaces dédiés à l'administrateur, au serveur et au livreur.

La solution repose sur \textbf{Flutter} pour l'interface Android, Windows et Web, et sur \textbf{Laravel} pour l'API REST. La version livrée est une version \textbf{démo-prod} adaptée à la soutenance : backend local, base SQLite seedée, comptes de test, sécurité des rôles, tests automatiques, guide HTML interactif et rapport LaTeX.
...
\begin{figure}[H]
  ...
  \caption{Aperçu des interfaces Flutter}
\end{figure}
